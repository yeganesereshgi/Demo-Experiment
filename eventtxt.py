...
def writeOutputFileHeader(output_file, session_metadata_columns,log_entry_names):
        ...
        """
        allcols =session_metadata_columns+log_entry_names
        s = ''   # EDIT CAT BELOW
        for element in allcols:
            s += (str(element) + '\t')
        output_file.write(s)    
        #output_file.write('\t'.join(allcols)) #EDIT CAT END
        output_file.write('\n')
...
def writeDataRow(output_file,session_info,session_uservar_names,event_data):
    ...
    all_data= session_data+[str(e) for e in event_data]# EDIT CAT
    #all_data= session_data+session_user_data+[str(e) for e in event_data]+session_user_data #original Code 
    s = ''   #EDIT CAT
    for element in all_data:
        s += (str(element) + '\t')
    output_file.write(s)    #EDIT CAT END
    #output_file.write('\t'.join(all_data))   #original code failing because of lists instead of strings
    output_file.write('\n')
class Eventtotext():
