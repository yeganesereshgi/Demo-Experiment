...
class ExperimentRuntime(ioHubExperimentRuntime):
    ...
    def run(self,*args):
        ...
        def to_output_eyetracking(subject_id, x, y, gazetime, trigger, output_eye_file_dir):
            ...
            # Python 3
            with open(output_eye_file_dir, 'a', newline = '') as f:
        ...
        def read_input_file(csv_dir, item_number):
            ...
            with open(csv_dir, 'rt') as csvfile:
        ...
        def draw_trigger(win, tracker, trigger, item_number, output_file_dir, output_eye_dir):
            ...
            #controller.start_recording('real-test.tsv')
        ...
        controller.start_recording('data_raw\\_EXP_DTA.tsv')
        ...
        tsv_dir_1 = 'data_raw\\_Calib_DTA.tsv'
        csv_dir_1 = 'data_raw\\'+subject_id+'_Calib_DTA'+localtime+'.csv'
        tsv_dir_2 = 'data_raw\\_EXP_DTA.tsv'
        csv_dir_2 = 'data_raw\\'+subject_id+'_EXP_DTA'+localtime+'.csv'
        tsv_file_1 = open(tsv_dir_1)
        read_tsv_1 = csv.reader(tsv_file_1, delimiter="\t")
        data = list(csv.reader(tsv_file_1, delimiter="\t"))
        
        df = pd.read_csv(tsv_dir_1, delimiter="\t")
        df["Trigger"] = trigger_value
        df["Subject_ID"] = subject_id
        df["Calibration_status"] = calib_state
        df.to_csv(tsv_dir_1, index=False)
        
        df = pd.read_csv(tsv_dir_2, delimiter="\t")
        df["Trigger"] = trigger_value
        df["Subject_ID"] = subject_id
        df["Calibration_status"] = calib_state
        df.to_csv(tsv_dir_2, index=False)
        
        
        
        with open(csv_dir_1, 'w', newline='') as f:
            
            writer = csv.writer(f)
            writer.writerows(data)
            
        
        tsv_file_2 = open(tsv_dir_2)
        read_tsv = csv.reader(tsv_file_2, delimiter="\t")
        data = list(csv.reader(tsv_file_2, delimiter="\t"))
        with open(csv_dir_2, 'w', newline='') as f:
...
if __name__ == "__main__":
    ...
    def main(configurationDirectory):
        ...
        eye_tracker_config_files={
                                  ...
                                  'The Eye Tribe':'eyetracker_configs/iTribe_config.yaml', 'Tobii Eye Tracker': 'eyetracker_configs/tobii_config.yaml'
                                  }

        info = {'Eye Tracker Type': ['Select', 'LC Technologies EyeGaze','The Eye Tribe',
                                     'SMI iViewX','Tobii Eye Tracker']}
        ...
        localtime=localtime[11:16]+'pm-'+localtime[4:10]
        localtime=localtime.replace(":","_").replace(" ","_")
        if list(dlg_info.values())[0] =='Tobii Eye Tracker':
            #if TSH =< 0.3
            ...
            controller = tobii_controller(win1)
            # adjust properties
            ...
            Res = visual.TextStim(win1, text="press Space to Accept the calibration\n\n"
                "Press Up key to recalibrate\n", alignText='center', height=20, pos = [0,0],color=[255,255,255],colorSpace='rgb255')
            ...
            marker = visual.Rect(win1, width=20, height=20, autoLog=False)

            # Start recording.
            # filename of the data file could be define in this method or when creating an
            # tobii_controller instance
            controller.start_recording('data_raw\\_Calib_DTA.tsv')
            #waitkey = True
            timer = core.Clock()
            decab= False
            i = 0
            ...
            #for x in range(1):
            '''
            calibration with pressing key part
            '''
            ...
            while waitkey:
                ...
                #marker.draw()
                print('list status:',controller.calib_auto_status)
                Res.draw()
                win1.flip()
            '''
            waitkey = True
            while waitkey:
                ...
                for elem in controller.calib_auto_status:
                    if (elem ==1):
                        j += 1
                    if (j == 5):
                        calib_state = '1'
        ...
        #****************Run calibration for eyetibe****************************
        if list(dlg_info.values())[0] =='The Eye Tribe':
            ...
            '''while True:
                ...
                if fails_counter == 3: 
                    ...
                    break 
                    , 'Tobii Eye Tracker': 'eyetracker_configs/tobii_config.yaml'
                    '''
